...
- Move transactional, provenance, and join details into a conditionally loaded coordination-controls reference.
- Keep execution leases separate because scarce repeated execution remains a distinct hazard.
- Put Codex session-log mechanics in harness defaults rather than the general skill.
- Route models by observed task fit and dated capability evidence rather than role prestige.
- Keep `AWAITING_ORCHESTRATOR` narrow: it is for material contract tension, not routine status, optional refinements, or avoidable questions.
...
- When do conditional join records measurably prevent errors rather than merely improve auditability?
